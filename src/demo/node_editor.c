...
int node_editor(struct nk_context *ctx)
{
        ...
        NK_WINDOW_BORDER|NK_WINDOW_NO_SCROLLBAR|NK_WINDOW_MOVABLE|NK_WINDOW_CLOSABLE))
    {
        ...
        nk_layout_space_begin(ctx, NK_STATIC, total_space.h, nodedit->node_count);
        {
            ...
            while (it) {
                ...
                }
                {
                    ...
                    for (n = 0; n < it->output_count; ++n) {
                        ...
                        if (nk_input_click_in_rect(in, NK_BUTTON_LEFT, circle) && nk_input_mouse_down(in, NK_BUTTON_LEFT)) {
                        ...
                        if (nodedit->linking.active && nodedit->linking.node == it &&
                            ...
                            struct nk_vec2 l1 = in->mouse_pos;
                    ...
                    for (n = 0; n < it->input_count; ++n) {
                        ...
                        circle.w = 8; circle.h = 8;
                        nk_fill_circle(canvas, circle, nk_rgb(100, 100, 100));
                        if (nk_input_mouse_released(in, NK_BUTTON_LEFT) &&
                            nk_input_mouse_hover(in, circle) &&
            ...
            if (nodedit->linking.active && nk_input_mouse_released(in, NK_BUTTON_LEFT))
            {
                nodedit->linking.active = nk_false;
            ...
            if (nk_input_mouse_clicked(in, NK_BUTTON_LEFT, nk_layout_space_bounds(ctx))) {
                ...
                nodedit->bounds = nk_rect(in->mouse_pos.x, in->mouse_pos.y, 100, 200);
                while (it) {
                    struct nk_rect b = nk_layout_space_rect_to_screen(ctx, it->bounds);
                    b.x -= nodedit->scrolling.x;
                    b.y -= nodedit->scrolling.y;
                    if (nk_input_mouse_hover(in, b))
                        nodedit->selected = it;
                    it = it->next;
                }
            }
        ...
        if (nk_input_mouse_hover(in, nk_window_get_bounds(ctx)) && nk_input_mouse_down(in, NK_BUTTON_MIDDLE))
        {
            nodedit->scrolling.x += in->mouse_delta.x;
            nodedit->scrolling.y += in->mouse_delta.y;
        }
    }
    nk_end(ctx);
